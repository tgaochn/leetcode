...
class Solution:
    def reorderLogFiles(self, logs: List[str]) -> List[str]:
        tmp = []
        for logId, log in enumerate(logs):
            items = log.split(' ')
            ele = (int(items[1].isnumeric()), logId if items[1].isnumeric() else items[1:], log)
            tmp.append(ele)
        # print(tmp)
        tmp.sort()
        return [ele[2] for ele in tmp]
    # endFunc
# endClass
...
def func():
    ...
    expectedRlt[0] = ["let1 art can", "let3 art zero", "let2 own kit dig", "dig1 8 1 5 1", "dig2 3 6"]
