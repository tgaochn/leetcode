...
class Solution:
    def restoreIpAddresses(self, s: str) -> List[str]:
        ...
        def bt(sol, path, sele):
            # print(path, sele)
            if len(sele) > 4 or (sele and not sele[-1]) or (sele and len(sele[-1]) > 1 and sele[-1][0] == '0') or (sele and int(sele[-1]) > 255):
                return

            if not path and len(sele) == 4:
                sol.append('.'.join(sele))
                return

            for i in range(min(3, len(path))):
                bt(sol, path[i + 1:], sele + [path[:i + 1]])
...
def func():
    ...
    expectedRlt[0] = ["255.255.11.135", "255.255.111.35"]
    ...
    expectedRlt[3] = ["0.10.0.10", "0.100.1.0"]
