...
class Solution:
    def longestConsecutive(self, root: TreeNode) -> int:
        def dfs(curNode):
            ...
            candLis = []
            if curNode.right and curNode.right.val == curNode.val + 1:
                candLis.append(1 + dfs(curNode.right))
            elif curNode.right:
                candLis.append(dfs(curNode.right))

            if curNode.left and curNode.left.val == curNode.val + 1:
                candLis.append(1 + dfs(curNode.left))
            elif curNode.left:
                candLis.append(dfs(curNode.left))
            
            return 1 if not candLis else max(candLis)
