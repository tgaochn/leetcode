...
class Solution:
    def isToeplitzMatrix(self, matrix: List[List[int]]) -> bool:
        ...
        startLis = [(i, 0) for i in range(m)] + [(0, i) for i in range(1, n)]
        # print(startLis)
        for x, y in startLis:
            curNum = matrix[x][y]
            while x + 1 < m and y + 1 < n:
                nextNum = matrix[x + 1][y + 1]
                if curNum != nextNum:
                    return False
                x += 1
                y += 1
...
def func():
    ...
    for inputPara, enableInput, expectedRlt in allInput:
        if not enableInput or (not inputPara or not inputPara[0]): continue
