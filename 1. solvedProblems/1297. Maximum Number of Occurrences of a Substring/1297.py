...
class Solution:
    def maxFreq(self, s: str, maxLetters: int, minSize: int, maxSize: int) -> int:
        """
        sliding window, shortest substring
        template 2.0
        """
        if not s: return 0
        ...
        while r < n:
            ...
            if len(existHash) > maxLetters or winSize > maxSize:
                removeEle(s[l])
                l += 1

            elif winSize < minSize:
                addEle(s[r])
                r += 1

            else:
                if len(existHash) <= maxLetters and winSize <= maxSize and winSize >= minSize:
                    freqHash.setdefault(win, 0)
                    freqHash[win] += 1
                removeEle(s[l])
                l += 1
        ...
        # print(freqHash)
        return 0 if not freqHash else max(list(freqHash.items()), key=lambda x: x[1])[1]
    # endFunc
# endClass
