...
class Solution:
    def largestRectangleArea(self, heights: List[int]) -> int:
        ...
        for i in range(n):
            ...
            while myStack and myStack[-1][0] > curHeight:
                eleHeight, eleIdx = myStack.pop()
                # curWidth = i - eleIdx
                curWidth = i - myStack[-1][1] - 1  # !! 关键是这一步, 比较了与栈顶元素的距离而不是pop出来元素的距离
                curArea = curWidth * eleHeight
                maxArea = max(maxArea, curArea)
            myStack.append((curHeight, i))
            # print(myStack)
