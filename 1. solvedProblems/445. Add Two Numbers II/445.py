...
class Solution:
    def addTwoNumbers(self, l1: ListNode, l2: ListNode) -> ListNode:
        ...
        n = max(len(stack1), len(stack2))
        flag = False
        for _ in range(n):
            ele1 = 0 if len(stack1) == 0 else stack1.pop()
            ele2 = 0 if len(stack2) == 0 else stack2.pop()
            eleSum = ele1 + ele2 if not flag else ele1 + ele2 + 1
            ele = eleSum % 10
            rltStack.append(ele)
            flag = True if eleSum >= 10 else False
        if flag:
            rltStack.append(1)

        rltLinkedList = ListNode(rltStack.pop())
        curNode = rltLinkedList
        while rltStack:
            newNode = ListNode(rltStack.pop())
...
def func():
    ...
    inputParaLis1 = [
        ...
        # singleLinkedList.buildSingleList([7, 2, 4, 3])
        # binaryTree.buildTree([])
        # nTree.buildTree([])
    ]
    inputParaLis2 = [
        singleLinkedList.buildSingleList([9, 9])
        # singleLinkedList.buildSingleList([5, 6, 4])
    ]
    inputParaLis3 = [
        None,
    ]
