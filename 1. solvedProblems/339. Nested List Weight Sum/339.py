...
class Solution:
    def depthSum(self, nestedList: List[int]) -> int:
        """
        BFS没问题, 但是原题里面有挺恶心的结构, 懒得改了
        """
        self.rlt = 0
        def recurVisit(cur, depth):
            if not isinstance(cur, list):
                self.rlt += cur.getInteger() * depth
            else:
                for ele in cur.getList():
                    recurVisit(ele, depth + 1)
                
        recurVisit(nestedList, 0)
        return self.rlt
    # endFunc
# endClass
