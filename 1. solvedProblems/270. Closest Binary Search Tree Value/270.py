...
class Solution:
    def closestValue(self, root: TreeNode, target: float) -> int:
        ...
        def recurVisist(cur):
            if not cur: return
            if cur.val == target: 
                self.closestVal = cur.val
                return 
            
            curDist = abs(cur.val - target)
            if curDist < self.minDist:
                self.minDist = curDist
                self.closestVal = cur.val

            if target > cur.val and cur.right:
                recurVisist(cur.right)
            elif target < cur.val and cur.left:
                recurVisist(cur.left)
