...
class Solution:
    def findReplaceString(self, S: str, indexes: List[int], sources: List[str], targets: List[str]) -> str:
        """
        https://leetcode-cn.com/problems/find-and-replace-in-string/solution/pai-xu-ran-hou-cong-hou-wang-qian-ti-huan-by-rocky/
        按要替换的起点排序，然后从后往前替换，这样不用担心替换后起点的索引会变化
        """
        sList = list(S)
        indexHash = {}
        ...
        for pos in posList:
            ...
            if S[pos: pos + strLen] == sourceStr:
                sList[pos: pos + strLen] = targets[idx] # elements in list can be replaced like this
