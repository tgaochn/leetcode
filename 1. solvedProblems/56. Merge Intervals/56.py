...
class Solution:
    def merge(self, intervals: List[List[int]]) -> List[List[int]]:
        ...
        rlt = []
        mergeIntervalStart = intervals[0][0]
        mergeIntervalEnd = intervals[0][1]
        for interval in intervals:
            curStart = interval[0]
            curEnd = interval[1]

            if mergeIntervalEnd >= curStart:
                mergeIntervalEnd = max(mergeIntervalEnd, curEnd)
            else:
                rlt.append([mergeIntervalStart, mergeIntervalEnd])
                mergeIntervalStart = curStart
                mergeIntervalEnd = curEnd
        rlt.append([mergeIntervalStart, mergeIntervalEnd])
