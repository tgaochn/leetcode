...
class Solution:
    def smallestSubsequence(self, s: str) -> str:
        ...
        for idx, ele in enumerate(s):
            while mystack and mystack[-1][1] > ele and mystack[-1][0] < lastPosDic[mystack[-1][1]] and idx < lastPosDic[mystack[-1][1]] and ele not in stackEleSet:
                stackEleId, stackEle = mystack.pop()
                stackEleSet.remove(stackEle)

            if ele not in stackEleSet:
                mystack.append((idx, ele))
                stackEleSet.add(ele)
            # print(mystack)

        return ''.join(map(lambda x: x[1], mystack))
    # endFunc
# endClass
