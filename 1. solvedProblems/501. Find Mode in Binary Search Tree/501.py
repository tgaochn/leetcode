...
class Solution:
    def findMode(self, root: TreeNode) -> List[int]:
        ...
        def inorderTra(node):
            ...
            if self.lastEle is None:
                self.lastEle = node.val
                self.maxFreq = 1
            if self.lastEle == node.val:
                self.lastEleFreq += 1
            else:
                if self.lastEleFreq > self.maxFreq:
                    self.modeEleSet = set([self.lastEle])
                    self.maxFreq = self.lastEleFreq
                elif self.lastEleFreq == self.maxFreq:
                    self.modeEleSet.add(self.lastEle)
                self.lastEle = node.val
                self.lastEleFreq = 1
        ...
        inorderTra(root)
        if self.lastEleFreq > self.maxFreq:
            self.modeEleSet = [self.lastEle]
            self.maxFreq = self.lastEleFreq
        elif self.lastEleFreq == self.maxFreq:
            self.modeEleSet.add(self.lastEle)

        return list(self.modeEleSet)
    # endFunc
# endClass
...
def func():
    ...
    input1 = (
        ...
        binaryTree.buildTree([1, null, 2, 2])
        # nTree.buildTree([])
    )
    expectedRlt1 = [2]
