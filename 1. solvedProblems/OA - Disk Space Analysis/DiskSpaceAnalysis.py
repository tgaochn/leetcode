...
https://www.1point3acres.com/bbs/thread-685887-1-1.html
同 239
"""
import sys
...
class Solution:
    def segment(self, x, space):
        class MonoQueue():
            ...
            def pop(self, ele):
                if self.data and self.data[0] == ele:
                    self.data.popleft()
        ...
        rlt = -float('inf')
        myMonoQue = MonoQueue()
        for idx, ele in enumerate(space):
            if idx < x - 1:
                myMonoQue.push(ele)
            else:
                myMonoQue.push(ele)
                curMin = myMonoQue.getMin()
                myMonoQue.pop(space[idx - x + 1])
                rlt = max(rlt, curMin)
        return rlt
    # endFunc

    def segment1(self, x, space):
        from collections import deque
        ...
        n = len(space)
        for i in range(n):
            if dq and dq[0] < i - x + 1:
                dq.popleft()
            while dq and space[dq[-1]] > space[i]:
                dq.pop()
            dq.append(i)
            if i >= x - 1:
                largest = max(largest, space[dq[0]])
        return largest
    # endFunc
# endClass
