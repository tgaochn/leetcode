...
class Solution:
    def assignBikes(self, workers: List[List[int]], bikes: List[List[int]]) -> List[int]:
        distLis = []
        for i, posW in enumerate(workers):
            for j, posB in enumerate(bikes):
                dist = abs(posW[0] - posB[0]) + abs(posW[1] - posB[1])
                distLis.append((dist, i, j))

        distLis.sort()
        assignedWorkerSet = set()
        assignedBikeSet = set()
        rlt = [-1] * len(workers)
        for dist, workerId, bikeId in distLis:
            if workerId in assignedWorkerSet or bikeId in assignedBikeSet:
                continue
            rlt[workerId] = bikeId
            assignedWorkerSet.add(workerId)
            assignedBikeSet.add(bikeId)
