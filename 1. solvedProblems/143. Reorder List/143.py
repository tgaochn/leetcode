...
class Solution:
    def reorderList(self, head: ListNode) -> None:
        ...
        # ! fast-slow pointer to splie the linkedin list
        sp, fp = head, head
        pre = None
        ...
        def reverseLink(pre, cur):
            if not cur: return pre
            tmp = cur.next
            cur.next = pre
            pre = cur
            cur = tmp
            return reverseLink(pre, cur)
            
        p2 = reverseLink(None, sp)
        p1 = head
        p = head
        cnt = 0

        # ! join the two linked list
        while p1 or p2:
            if not p1: 
                p.next = p2
                break
...
def func():
    ...
    input[0] = (
        ...
        singleLinkedList.buildSingleList([1, 2, 3, 4])
        # nTree.buildTree(None)
    )
    expectedRlt[0] = singleLinkedList.buildSingleList([1, 4, 2, 3])
