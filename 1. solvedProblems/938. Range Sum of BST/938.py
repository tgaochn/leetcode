...
class Solution:
    def rangeSumBST(self, root: TreeNode, low: int, high: int) -> int:
        ...
        def recurSum(node, low, high):
            if node.val >= low and node.val <= high:
                self.rlt += node.val
            if node.left and low < node.val:
                recurSum(node.left, low, high)
            if node.right and high > node.val:
                recurSum(node.right, low, high)

        recurSum(root, low, high)
        return self.rlt
    # endFunc
# endClass
...
def func():
    ...
    # ! para1
    input[0] = (
        binaryTree.buildTree([10, 5, 15, 3, 7, null, 18]),
        7,
        15,
    ...
    # ! para2
    input[1] = (
        binaryTree.buildTree([15, 9, 21, 7, 13, 19, 23, 5, null, 11, null, 17]),
        19,
        21,
        # singleLinkedList.buildSingleList(None),
        # nTree.buildTree(None),
