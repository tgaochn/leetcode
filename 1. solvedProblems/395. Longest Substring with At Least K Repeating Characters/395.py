...
class Solution:
    def longestSubstring(self, s: str, k: int) -> int:
        ...
        divide and conquer
        """
        def getSubstringDQ(s):
            freqHash = {}
            invalidCharSet = set()
            validCharSet = set()
            for c in s:
                freqHash.setdefault(c, 0)
                freqHash[c] += 1
                if freqHash[c] < k:
                    invalidCharSet.add(c)
                else:
                    validCharSet.add(c)
                    if c in invalidCharSet:
                        invalidCharSet.remove(c)

            n = len(s)
            if not invalidCharSet:
                return n
            ...
            maxLen = -float('inf')
            while r < n:
                if s[r] in invalidCharSet:
                    curSubString = s[l: r]
                    maxLen = max(maxLen, getSubstringDQ(curSubString))
                    l = r + 1
                r += 1
            curSubString = s[l: r]
            maxLen = max(maxLen, getSubstringDQ(curSubString))
