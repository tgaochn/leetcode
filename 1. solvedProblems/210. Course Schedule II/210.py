...
class Solution:
    def findOrder(self, numCourses: int, prerequisites: List[List[int]]) -> List[int]:
        ...
        adjHash = {cid: [] for cid in range(numCourses)}
        inDegreeHash = {cid: 0 for cid in range(numCourses)}
        for coursePair in prerequisites:
            fromCid = coursePair[1]
            toCid = coursePair[0]
            adjHash[fromCid].append(toCid)
            inDegreeHash[toCid] += 1

        # print(adjHash)
        # print(inDegreeHash)

        myQue = deque([])
        courseOrder = [float('inf')] * numCourses
        curOrd = 0
        for cid, inDegree in inDegreeHash.items():
            if inDegree == 0:
                myQue.append(cid)
                courseOrder[cid] = curOrd
                curOrd += 1

        while myQue:
            curCid = myQue.popleft()
            nextCidList = adjHash[curCid]
            for nextCid in nextCidList:
                inDegreeHash[nextCid] -= 1
                if inDegreeHash[nextCid] == 0:
                    myQue.append(nextCid)
                    courseOrder[nextCid] = curOrd
                    curOrd += 1

        return [] if curOrd != numCourses else courseOrder
    # endFunc
# endClass
...
def func():
    ...
    # ! para1
    input[0] = parsePara('numCourses = 2, prerequisites = [[1,0]]')
    # input[0] = (
        # None,
    # )
    expectedRlt[0] = [0, 1]
    ...
    # ! para2
    input[1] = parsePara('numCourses = 4, prerequisites = [[1,0],[2,0],[3,1],[3,2]]')
    # input[1] = (
        # None,
    # )
    expectedRlt[1] = [0, 2, 1, 3]
