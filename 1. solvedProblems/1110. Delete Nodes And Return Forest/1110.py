...
class Solution:
    def delNodes(self, root: TreeNode, to_delete: List[int]) -> List[TreeNode]:
        """
        只能用post-order, 不然子节点中可能有应该被删除的节点
        必须先子节点后父节点
        """
        rlt = []

        def dfs(cur, parent, dir): # 需要记录父节点和父节点过来的方向以便删除(即给对应节点赋None)
            if not cur:
                return
            ...
            if cur.val in to_delete:
                if cur.left:
                    rlt.append(cur.left)
                if cur.right:
                    rlt.append(cur.right)
                if dir == 'left':
                    parent.left = None
                if dir == 'right':
                    parent.right = None

            if dir is None and cur.val not in to_delete: # 最后加上原本的树, 标志是 dir 为 None
                rlt.append(root)
