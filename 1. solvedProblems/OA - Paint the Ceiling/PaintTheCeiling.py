...
CreationDate:
    Sat, 11/14/2020, 14:33
# !! Description:
题目:
https://www.1point3acres.com/bbs/forum.php?mod=viewthread&tid=578884&highlight=snowflake
SnowflakeOA.zip
...
class Solution:
    def variantsCount(self, n, s0, k, b, m, a):
        """
        双指针
        """
        s = [s0]
        for i in range(1, n):
            si = ((k * s[i - 1] + b) % m) + 1 + s[i - 1]
            s.append(si)
