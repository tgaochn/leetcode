...
class Solution:
    def alphabetBoardPath(self, target: str) -> str:
        ...
        for i in range(26):
            ...
            row = i // 5
            col = i % 5
            char2pos[curChar] = (row, col)

        # print(char2pos)
        curRow = 0
        curCol = 0
        rlt = []
        for curChar in target:
            row, col = char2pos[curChar]
            while curRow != row or curCol != col:
                if row < curRow: # 因为Z的特殊位置, 需要 U > R, L > D
                    rlt.append('U')
                    curRow -= 1
                elif col > curCol:
                    rlt.append('R')
...
def func():
    ...
    expectedRlt[0] = "DDR!UURRR!!DDD!"

    # ! para2
    input[1] = (
        "code",
    )
    expectedRlt[1] = "RR!DDRR!UUL!R!"

    # ! para3
    input[2] = (
        "zdz",
    )
    expectedRlt[2] = "DDDDD!UUUUURRR!DDDDLLLD!"
