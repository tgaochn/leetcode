...
CreationDate:
    Sun, 11/15/2020, 18:29
# !! Description:
题目:
https://instant.1point3acres.com/thread/681268
...
class Solution:
    def largestArea(self, samples):
        ...
        for i in range(m):
            dp[i][0] = 1 if samples[i][0] == '1' else 0
            maxLen = max(maxLen, dp[i][0])
        for i in range(n):
            dp[0][i] = 1 if samples[0][i] == '1' else 0
