...
class Solution:
    def isNStraightHand(self, hand: List[int], W: int) -> bool:
        """
        https://leetcode-cn.com/problems/hand-of-straights/solution/yi-shou-shun-zi-by-leetcode/
        变成hash, 每次取最小, 多次扫描即可
        """
        n = len(hand)
        ...
        handHash = {}
        for card in hand:
            handHash.setdefault(card, 0)
            handHash[card] += 1

        while handHash:
            minCard = min(handHash.keys())
            for i in range(W):
                curCard = minCard + i
                if curCard not in handHash: return False
                handHash[curCard] -= 1
                if handHash[curCard] == 0:
                    del handHash[curCard]
