...
class Solution:
    def nextGreaterElement(self, nums1: List[int], nums2: List[int]) -> List[int]:
        """
        单调栈: 解决下一个更大的数问题
        先求nums2每个元素的下一个更大元素, 建立字典在遍历nums1
        """
        myStack = []
        nextGreaterEleDic = {}
        rlt = []
        ...
        for idx, ele in enumerate(nums2):
            while myStack and myStack[-1][1] < ele:
                stackEleIdx, stackEle = myStack.pop()
                nextGreaterEleDic[stackEle] = ele
            myStack.append((idx, ele))

        for ele in nums1:
            NGE = nextGreaterEleDic.get(ele, -1)
            rlt.append(NGE)
    ...
    def nextGreaterElement1(self, nums1: List[int], nums2: List[int]) -> List[int]:
        ...
        暴力
        """
        ele2idx = {ele: idx for idx, ele in enumerate(nums2)}

        n1 = len(nums1)
        n2 = len(nums2)
        rlt = [-1] * n1
        for i, ele1 in enumerate(nums1):
            startIdx = ele2idx[ele1]
            for j in range(startIdx + 1, n2):
                ele2 = nums2[j]
                if ele2 > ele1:
                    rlt[i] = ele2
                    break
