...
class Solution:
    def sortedArrayToBST(self, nums: List[int]) -> TreeNode:
        ...
        def buildBST(node, l, r):
            if l > r: return
            m = l + (r - l) // 2
            node = TreeNode(nums[m])
            node.left = buildBST(node.left, l, m - 1)
            node.right = buildBST(node.right, m + 1, r)
            return node
        
        root = buildBST(root, l, r)
        return root
    # endFunc
# endClass
...
def func():
    ...
    input1 = (
        ...
        # binaryTree.buildTree([-10,-3,0,5,9])
