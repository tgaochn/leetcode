...
class Solution:
    def numberOfTokens(self, expiryLimit, commands):
        ...
        https://github.com/maverick009/Leetcode/blob/master/000.%20Twitter%20-%20Authentication%20Tokens/src/main/Main.java
        就按照要求过一遍循环即可...
        """
        myHash = {}
        tokenCnt = 0
        ...
        for cmd in commands:
            ...
            tokenId = cmd[1]
            cmdTime = cmd[2]
            curTime = max(curTime, cmdTime)

            if cmdType == 0 and tokenId in myHash:
                continue
            if cmdType == 1 and myHash.get(tokenId, -float('inf')) < cmdTime:
                continue
            
            myHash[tokenId] = cmdTime + expiryLimit
            
        for tokenId, expiryTime in myHash.items():
