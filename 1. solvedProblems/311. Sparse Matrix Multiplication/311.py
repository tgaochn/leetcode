...
class Solution:
    def multiply(self, A: List[List[int]], B: List[List[int]]) -> List[List[int]]:
        m = len(A)
        n = len(B[0])
        rlt = [[0] * n for _ in range(m)]

        valHashA = {}
        for row, elements in enumerate(A):
            for col, ele in enumerate(elements):
                if not ele: continue
                valHashA.setdefault(row, {})
                valHashA[row].setdefault(col, ele)

        valHashB = {}
        for row, elements in enumerate(B):
            for col, ele in enumerate(elements):
                if not ele: continue
                valHashB.setdefault(col, {})
                valHashB[col].setdefault(row, ele)

        for i in range(m):
            for j in range(n):
                if i not in valHashA or j not in valHashB: continue
                curRow = valHashA[i]
                curCol = valHashB[j]
                for k, v in curRow.items():
                    if k not in curCol: continue
                    rlt[i][j] += v * curCol[k]
