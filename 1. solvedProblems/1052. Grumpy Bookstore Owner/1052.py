...
class Solution:
    def maxSatisfied(self, customers: List[int], grumpy: List[int], X: int) -> int:
        ...
        n = len(customers)
        unsatCustomers = [customers[i] if grumpy[i] == 1 else 0 for i in range(n)]
        ...
        while r < n:
            ...
            eleL = unsatCustomers[l]
            cnt -= eleL
            myQue.popleft()
            l += 1
        maxCnt = max(maxCnt, cnt)

        return maxCnt + sum(customers) - sum(unsatCustomers)
    # endFunc
# endClass
