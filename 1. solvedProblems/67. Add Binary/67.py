...
class Solution:
    def addBinary(self, a: str, b: str) -> str:
        ...
        rlt = []
        for i in range(n):
            digit1 = int(a[-i - 1]) if i < len(a) else 0
            digit2 = int(b[-i - 1]) if i < len(b) else 0
            # print(digit1)
            # print(digit2)
            addRlt = (digit1 + digit2 + flag) % 2
            flag = (digit1 + digit2 + flag) // 2
            rlt.append(str(addRlt))
        if flag:
            rlt.append(str(flag))
        rlt.reverse()
        return ''.join(rlt)
