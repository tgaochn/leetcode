...
class Solution:
    def countNumbersWithUniqueDigits(self, n: int) -> int:
        ...
        def bt(restDigitCnt, availDigitSet):
            # print(availDigitSet)
            # prune

            if restDigitCnt == 1:
                self.sol += len(availDigitSet)
                return
            
            if restDigitCnt == 0:
                return
            
            availDigitSetCopy = deepcopy(availDigitSet)
            for curDigit in availDigitSet:
                if len(availDigitSet) == 10 and curDigit == 0: continue
                availDigitSetCopy.remove(curDigit)
                bt(restDigitCnt - 1, availDigitSetCopy)
                availDigitSetCopy.add(curDigit)
        
        self.sol = 0
        for i in range(1, n + 1):
            bt(i, set(list(range(10))))
        return self.sol
    # endFunc
# endClass
...
def func():
    ...
    s = Solution()
    myFuncLis = [
        s.countNumbersWithUniqueDigits,
