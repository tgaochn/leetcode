...
class Solution:
    def partition(self, s: str) -> List[List[str]]:
        ...
        def bt(sol, start, end):
            if end == n:
                subStrLen = sum([len(subStr) for subStr in sol])
                if subStrLen != n: return
                rlt.append(sol)
                return

            bt(sol, start, end + 1)
            curSeg = s[start:end + 1]
            if isPalindrome(curSeg):
                bt(sol + [curSeg], end + 1, end + 1)
