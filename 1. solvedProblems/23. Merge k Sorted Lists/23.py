...
class Solution:
    def mergeKLists(self, lists: List[ListNode]) -> ListNode:
        ...
        n = len(lists)
        curPt = [node for node in lists if node]
        myHeap = [(node.val, i) for i, node in enumerate(curPt)]
        heapq.heapify(myHeap)
        rlt = None
        rltPt = None
        
        while myHeap:
            minEle, idx = heapq.heappop(myHeap)
            # rlt.append(minEle)
            ...
            if not rlt:
                ...
                rltPt.next = ListNode(minEle)
                rltPt = rltPt.next
                
            if curPt[idx].next:
                curPt[idx] = curPt[idx].next
                heapq.heappush(myHeap, (curPt[idx].val, idx))
...
def func():
    ...
    input[0] = (
        [
            singleLinkedList.buildSingleList([1, 4, 5]), 
            singleLinkedList.buildSingleList([1, 3, 4]), 
            singleLinkedList.buildSingleList([2,6]), 
         ],
    )
    expectedRlt[0] = None
