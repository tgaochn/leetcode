...
class Solution:
    def minRemoveToMakeValid(self, s: str) -> str:
        stack = []
        removedId = set()
        for idx, curChar in enumerate(s):
            if curChar == ')':
                if not stack:
                    removedId.add(idx)
                else:
        ...
        for idx in stack:
            rlt[idx] = None
        for idx in removedId:
            rlt[idx] = None
        # print(rlt)
        rlt = [ele for ele in rlt if ele is not None]
        return ''.join(rlt)
    # endFunc
# endClass
