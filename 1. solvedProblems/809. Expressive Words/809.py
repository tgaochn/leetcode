...
class Solution:
    def expressiveWords(self, S: str, words: List[str]) -> int:
        """
        https://leetcode-cn.com/problems/expressive-words/solution/809-qing-gan-feng-fu-de-wen-zi-by-haoyuhu/
        统计频次然后逐一比较即可
        如 heeellooo 统计结果为 [[h,1], [e,3], [l,1], [o,3]]
        """
        def formatWord(word):
            formattedCharFreq = []
            lastChar = ''
            ...
            for curChar in word:
                if not lastChar:
                    lastChar = curChar
                    freq = 1
                elif lastChar != curChar:
                    formattedCharFreq.append((lastChar, freq))
                    lastChar = curChar
                    freq = 1
                else:
                    freq += 1
            formattedCharFreq.append((lastChar, freq))
            return formattedCharFreq

        targetWordCharFreq = formatWord(S)
        # print(targetWordCharFreq)

        rlt = 0
        for word in words:
            curWordCharFreq = formatWord(word)
            if len(curWordCharFreq) != len(targetWordCharFreq):
                continue
            flag = 1
            for i in range(len(curWordCharFreq)):
                curChar, curFreq = curWordCharFreq[i]
                tarChar, targetFreq = targetWordCharFreq[i]
                if curChar != tarChar or (targetFreq < 3 and curFreq < targetFreq) or curFreq > targetFreq:
                    flag = 0
                    break
            if flag:
                rlt += 1
